...
folder_info <- drive_ls(as_id("1Oeliwo45KmeU_l-8AFHvT0Lp1PSUShjl")) %>% 
  select(name, id)

# Function to process files in each folder
process_folder_files <- function(folder_id, folder_name) {
  # Create the directory path
  dir_path <- paste0("~/digipols/uk_2024/data2/", folder_name, "/")
  ...
  df <- map_df(file_paths, ~ read_excel(.x) %>%
    ...
    select(-c(party, Theme)) %>%
    unite("id1", c(ID, Item), sep = ") ") %>%
    relocate(coder, .before = Source)
  
  # Save the dataframe
  save(df, file = here("data2", folder_name, "df.rda"))
}
